...
import { FileText, ChevronRight, Sparkles, ArrowRight, TrendingUp } from 'lucide-react';
...
const segments: WheelSegment[] = [
  { number: 1, color: '#ef4444', title: 'Mentalidad Ganadora', description: 'El éxito comienza en la mente.' },
  { number: 2, color: '#f97316', title: 'Metas Claras', description: 'Define objetivos específicos.' },
  { number: 3, color: '#f59e0b', title: 'Plan de Acción', description: 'Crea un roadmap detallado.' },
  { number: 4, color: '#84cc16', title: 'Ingresos Múltiples', description: 'Diversifica tus ingresos.' },
  { number: 5, color: '#22c55e', title: 'Inversión Inteligente', description: 'Retornos pasivos consistentes.' },
  { number: 6, color: '#10b981', title: 'Red de Contactos', description: 'Construye relaciones valiosas.' },
...
export default function PresentacionE2() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('ingenio-materials-E2');
    if (saved) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Presentación E2 - Avanzado</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Button variant="ghost" onClick={() => window.location.href = '/presentacion-e1'}>
              Ver E1
            </Button>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-2">
            El Sistema de la Riqueza Real
          </h2>
          <p className="text-slate-600 dark:text-slate-400">
            Gira la ruleta para descubrir tu próximo paso hacia la maestría financiera
          </p>
        </div>
                ...
                <Card key={mat.id} className="group hover:shadow-lg transition-shadow border-slate-200 dark:border-slate-700">
                  <CardContent className="p-4">
                    ...
                    <a 
                      ...
                      className="mt-4 w-full inline-flex items-center justify-center px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium transition-colors"
                    >
